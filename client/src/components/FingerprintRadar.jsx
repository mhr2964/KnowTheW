// Hand-rolled 6-spoke radar for the archetype card (no chart lib in the project — a radar is just a
...
const N = 6;             // spokes — must match dimensions length

// Point on spoke i at `pct` of full radius. Spoke 0 points up (−90°), then clockwise every 60°.
...
export default function FingerprintRadar({ dimensions }) {
  if (!Array.isArray(dimensions) || dimensions.length !== N) return null;

  const val = (d) => (typeof d.value === 'number' ? d.value : 0);
  const ringPoints = (pct) => dimensions.map((_, i) => polar(i, pct).join(',')).join(' ');
  const dataPoints = dimensions.map((d, i) => polar(i, val(d)).join(',')).join(' ');

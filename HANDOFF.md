...
- **`notifications` collection now has TWO unique indexes for two different doc shapes** (`db.js`) — the original `{userId, gameId}` plain unique index for game docs, and a NEW partial unique index on `{userId, playerId, status}` scoped to `type:'injury'` (Feature 11) for injury docs, since they have no gameId and would otherwise falsely collide with each other. Adding a THIRD notification type needs its own scoped idempotency key the same way, not a reuse of either existing index.
- **Heroku Scheduler add-on is not installed on this app** (`heroku addons` returns none, confirmed live 2026-08-22) — both `/internal/jobs/notifications/poll` (pre-game, pre-existing) and `/internal/jobs/notifications/injuries/poll` (Feature 11) are built, tested, and gated by `requireSchedulerAuth`, but neither has a production trigger. They're dormant, not broken — don't assume either job has ever actually run against real users.

...
last-session: 2026-08-17 (BallDontLie hybrid provider: Phase 1 + Phase 2 built and fully tested (18 new test files/blocks, 249/249 suite green), shadow-compared live against ESPN for a star and a bench player across 4 player-seasons -- results match within noise. A rate-limit robustness bug was found and fixed during shadow-compare (see Traps). Not yet flipped on in production.)
state: BDL hybrid provider is code-complete, unit-tested, and shadow-compared clean; only the production STATS_PROVIDER flip remains (task #10 in the plan file) -- a production change requiring explicit go-ahead, not done unilaterally
```
...
**BallDontLie GOAT-tier provider swap: implementation + verification done, rollout not started.** Full plan (context, decisions, spike findings, both phases, the bugs found mid-build and during shadow-compare, and the shadow-compare results) lives at `C:\Users\Owner\.claude\plans\can-we-plan-out-polished-catmull.md` — read that before continuing, it's the authoritative reference, not this file. Short version: `server/providers/balldontlie/` implements the full `SportsDataProvider` contract as a hybrid facade (ESPN for everything pre-2008 and for identity/roster/schedule always; BDL for team stats + PBP-derived stats at season ≥ 2008). Activate via `STATS_PROVIDER=balldontlie` — currently unset in production (still ESPN). Live shadow-compare (A'ja Wilson + Kierstan Bell, 4 player-seasons) matched ESPN within noise on every advanced stat. Only remaining gap: an early-2008-era player spot-check wasn't run (lower priority, narrow historical slice). Next step is the production env-var flip on Heroku, whenever the user wants to pull that trigger.

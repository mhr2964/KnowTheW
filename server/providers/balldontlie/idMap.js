...
// Reverse of the above: a BDL bulk endpoint (e.g. /player_shot_locations) hands back a plain name,
// not an ESPN id, and every player-linking UI on this site routes by ESPN id. Rather than a name
// search per row (this codebase's forward direction, one BDL API call per player), build one
// in-process name->id index from sources this app already holds locally: getActivePlayers() (in
// memory) and the playerIndex Mongo collection (id/name for ~every historical player, same source
// percentileClient.js's loadPosMap already reads). Memoized for the process lifetime -- identity
// doesn't change at runtime, same rationale as teamMapPromise above.
...
// Exact full-name match only, same ambiguous-collision-drops-to-null discipline as
// matchPlayerCandidate above (just walked backwards: name -> id here, id -> name there). Returns
// null for zero or multiple matches -- a wrong link would misattribute a real leaderboard row to
// the wrong player, worse than the row just showing a name with no link.
...
module.exports = {
  isRealFranchise, getBdlTeams, resolveBdlTeamId, resolveBdlPlayerId, resolveEspnIdByName,

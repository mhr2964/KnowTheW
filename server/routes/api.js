...
router.get('/players/:id/advanced-pbp-all', async (req, res) => {
  try {
    ...
    if (db) {
      ...
      if (advCached?.gp === currentGP && advCached.v === 6 && advCached.data?.regular != null) return res.json(advCached.data);
    }
    ...
    if (db) db.collection('advancedStats')
      .replaceOne({ _id: req.params.id }, { _id: req.params.id, gp: currentGP, v: 6, data: advResult }, { upsert: true })
